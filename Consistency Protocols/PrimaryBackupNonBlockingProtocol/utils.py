...
async def rpc_primary(ip_address, operation, fileid, filename, content):
    channel=grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.ServerStub(channel)
    request = blocking_pb2.PrimaryRequest(
        operation=operation,
        fileid=fileid,
        filename=filename,
        content=content
    )
    response = await stub.Primary(request)
    return response


async def rpc_server_write(ip_address, filename, content, fileid):
    channel=grpc.aio.insecure_channel(ip_address) 
    stub = blocking_pb2_grpc.ServerStub(channel)
    request=blocking_pb2.WriteRequest(filename=filename, content=content, fileid=fileid)
    response=None
    try:
        response = await stub.Write(request)
    finally:
        pass #await channel.close()
    return response


async def rpc_server_delete(ip_address, fileid):
    channel=grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.ServerStub(channel)
    request=blocking_pb2.DeleteRequest(fileid=fileid)
    response=None
    try:
...
async def rpc_client_read(ip_address, fileid):
    channel=grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.ServerStub(channel)
    request=blocking_pb2.ReadRequest(fileid=fileid)
    response=None
    try:
        response =await stub.ReadReq(request)
    finally:
        pass #await channel.close()
    return response


async def rpc_client_delete(ip_address, fileid):
    channel=grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.ServerStub(channel)
    response=None
    request=blocking_pb2.DeleteRequest(fileid=fileid)
    try:
        response = await stub.DeleteReq(request)
    finally:
        pass #await channel.close()
    return response


async def rpc_client_write(ip_address, filename, content, fileid):
    channel= grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.ServerStub(channel)
    request=blocking_pb2.WriteRequest(filename=filename, content=content, fileid=fileid)
    response=None
    try:
        response = await stub.WriteReq(request)
    finally:
        pass #await channel.close()
    return response


async def rpc_registry_get_servers(ip_address):
    channel= grpc.aio.insecure_channel(ip_address)
    stub = blocking_pb2_grpc.RegistryServerStub(channel)
    request = blocking_pb2.ServerListRequest()
    response=None
    try:
        response = await stub.GetServerList(request)
    finally:
        pass #await channel.close()
