...
def map_rpc(port, path):
    ...
    with grpc.insecure_channel(ipaddress) as channel:
        stub = wordcount_pb2_grpc.MapperStub(channel)
        stub.Map(wordcount_pb2.ProcessRequest(path=path))


def partition_rpc(port):
    ipaddress = f'localhost:{port}'
    with grpc.insecure_channel(ipaddress) as channel:
        stub = wordcount_pb2_grpc.ReducerStub(channel)
        stub.Partition(wordcount_pb2.ProcessRequest())
...
def reduce_rpc(port):
    ...
    with grpc.insecure_channel(ipaddress) as channel:
        stub = wordcount_pb2_grpc.ReducerStub(channel)
        stub.Reduce(wordcount_pb2.ProcessRequest())
